...
// CODE HERE
const totalPopulation = populations.reduce((acc, cur) => {
  return acc + cur
})
...
// CODE HERE
const afterTaxArr = orders.map((element) => {
  return element.price = element.price * (1 + element.tax)
})

// console.log(afterTaxArr)
...
const bobsTotal = purchases.filter((element) => element.owner === "Bob").reduce((acc, cur) => {
